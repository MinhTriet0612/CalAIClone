...
describe('UC-11: View Meal History (e2e)', () => {
    ...
    await prisma.meal.createMany({
      data: [
        ...
        },
        {
          profileId,
          name: 'Bữa hôm qua 1',
          calories: 600, protein: 40, carbs: 50, fats: 15,
          date: yesterday,
          foodItems: ['Bún'],
        },
        {
          profileId,
          name: 'Bữa hôm qua 2',
          calories: 400, protein: 20, carbs: 30, fats: 5,
          date: yesterday,
          foodItems: ['Phở'],
        },
        {
          ...
          calories: 800, protein: 50, carbs: 60, fats: 20,
          date: twoDaysAgo,
  ...
  describe('FR_11.3: Lấy, nhóm bữa ăn và tính tổng', () => {
    it('TC_BB_11.3.1, TC_BB_11.3.2, TC_BB_11.3.3 - Nhóm đúng và tính tổng chuẩn xác', async () => {
      ...
      const yesterdaySummary = res.body.find((s: any) => s.date === yesterdayStr);
    ...
    it('TC_BB_11.3.4 - startDate > endDate -> Báo lỗi', async () => {
      const res = await request(app.getHttpServer())
  ...
  describe('FR_11.4: Sắp xếp kết quả', () => {
    it('TC_BB_11.4.1 - Sắp xếp từ ngày mới nhất đến ngày cũ nhất (giảm dần)', async () => {
      const today = new Date();
      const todayStr = today.toISOString().split('T')[0];
      const twoDaysAgoStr = new Date(today.getTime() - 86400000 * 2).toISOString().split('T')[0];

      const res = await request(app.getHttpServer())
        .get(`/api/meals/history?startDate=${twoDaysAgoStr}&endDate=${todayStr}`)
        .set('Authorization', `Bearer ${token}`);
